...
def enhanced_conceptual_kg(out_png: Path) -> None:
    ...
    for source, target, relation, color in edges:
        if source in nodes and target in nodes:
            ...
            if relation in ["causally_related_to", "triggers", "leads_to"]:
                linestyle = "-"
                linewidth = 2.5
                alpha = 0.8
            elif relation in ["involves", "affects", "damages"]:
                linestyle = "-"
                linewidth = 2.0
                alpha = 0.7
            elif relation in ["encodes", "forms", "participates_in"]:
                linestyle = "-"
                linewidth = 1.8
                alpha = 0.6
            else:  # evidence relationships
                linestyle = "--"
                linewidth = 1.2
                alpha = 0.4
...
def realistic_pipeline_overview(out_png: Path) -> None:
    """Create a realistic pipeline overview with feedback loops and metrics."""
    fig, ax = plt.subplots(figsize=(14, 10))

    # Pipeline stages with feedback loops
    stages = {
        # Input stage
        "Data_Ingestion": (2, 8),
        "Quality_Control": (2, 6.5),
        "Validation": (2, 5),
        # Processing stage
        "Feature_Extraction": (5, 8),
        "Prior_Integration": (5, 6.5),
        "KG_Construction": (5, 5),
        # Analysis stage
        "ML_Ranking": (8, 8),
        "Causal_Analysis": (8, 6.5),
        "Uncertainty_Quantification": (8, 5),
        # Output stage
        "Biomarker_Ranking": (11, 8),
        "Experimental_Design": (11, 6.5),
        "Validation_Planning": (11, 5),
        # Feedback/reinforcement
        "Performance_Monitoring": (6.5, 3),
        "Model_Updating": (6.5, 1.5),
    }
